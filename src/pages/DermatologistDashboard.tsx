...
export default function DermatologistDashboard() {
  ...
  useEffect(() => {
    const fetchQueue = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/v1/clinical-workflow/queue/dermatologist`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/v1/clinical-workflow/stats/dermatologist`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('access_token')}`
          }
        });
